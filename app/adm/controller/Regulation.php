...
use app\adm\validate\RegulationValidate;
    ...
    public function add()
    {
        ...
        if (request()->isAjax()) {
            if (!empty($param['id']) && $param['id'] > 0) {
                try {
                    validate(RegulationValidate::class)->scene('edit')->check($param);
                ...
                try {
                    validate(RegulationValidate::class)->scene('add')->check($param);
                } catch (ValidateException $e) {
                    // 验证失败 输出错误信息
                    return to_assign(1, $e->getError());
                }
