...
class Official extends BaseController
{
    ...
    */
    ...
    public function number_list()
    {
        if (request()->isAjax()) {
			$types=['-','收文','发文'];
            $list = Db::name('DocBase')->order('id asc')->select()->toArray();
			foreach ($list as $key => &$value){
				$value['types_str']=$types[$value['types']];
			}
    ...
    public function number_add()
    {
        ...
        if (request()->isAjax()) {
            if (!empty($param['id']) && $param['id'] > 0) {
                try {
                    validate(DocBaseCheck::class)->scene('edit')->check($param);
                } catch (ValidateException $e) {
                    // 验证失败 输出错误信息
                    return to_assign(1, $e->getError());
                }
                $param['update_time'] = time();
                $res = Db::name('DocBase')->strict(false)->field(true)->update($param);
                if ($res) {
                ...
                try {
                    validate(DocBaseCheck::class)->scene('add')->check($param);
                } catch (ValidateException $e) {
                    // 验证失败 输出错误信息
                    return to_assign(1, $e->getError());
                }
                $param['create_time'] = time();
                $insertId = Db::name('DocBase')->strict(false)->field(true)->insertGetId($param);
                if ($insertId) {
			...
			$id = isset($param['id']) ? $param['id'] : 0;
            if ($id > 0) {
                $detail = Db::name('DocBase')->where(['id' => $id])->find();
                View::assign('detail', $detail);
            }
            View::assign('id', $id);
            return view();
    ...
    //查看
    public function number_view()
    {
        $param = get_params();
		$id = isset($param['id']) ? $param['id'] : 0;
		$types=['-','收文','发文'];
        $detail = Db::name('DocBase')->where(['id' => $id])->find();
		$detail['types_str']=$types[$detail['types']];
        View::assign('detail', $detail);
        return view();
    }
    ...
    {
		$param = get_params();
        $res = Db::name('DocBase')->strict(false)->field('id,status')->update($param);
		if ($res) {
			if($param['status'] == 0){
