...
class Problemscate extends BaseController
{
	/**
     * 构造函数
     */
    ...
    public function __construct()
    {
        ...
        $this->model = new ProblemsCateModel();
    }
    ...
    public function add()
    {
        ...
        if (request()->isAjax()) {		
            if (!empty($param['id']) && $param['id'] > 0) {
                try {
                    validate(ProblemsCateValidate::class)->scene('edit')->check($param);
                ...
                try {
                    validate(ProblemsCateValidate::class)->scene('add')->check($param);
                } catch (ValidateException $e) {
                    // 验证失败 输出错误信息
                    return to_assign(1, $e->getError());
                }
