...
use app\service\model\ProblemsWork as ProblemsWorkModel;
use app\service\validate\ProblemsWorkValidate;
use think\exception\ValidateException;
use think\facade\Db;
use think\facade\View;

class Problemswork extends BaseController
{
	/**
     * 构造函数
     */
    ...
    public function __construct()
    {
        ...
        $this->model = new ProblemsWorkModel();
    }
    ...
    public function add()
    {
        ...
        if (request()->isAjax()) {
            ...
            if (!empty($param['id']) && $param['id'] > 0) {
                try {
                    validate(ProblemsWorkValidate::class)->scene('edit')->check($param);
                ...
                try {
                    validate(ProblemsWorkValidate::class)->scene('add')->check($param);
                } catch (ValidateException $e) {
                    // 验证失败 输出错误信息
                    return to_assign(1, $e->getError());
                }
