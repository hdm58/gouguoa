...
use think\model;
use think\facade\Db;
class ProblemsWork extends Model
{
    /**
    * 获取分页列表
    * @param $where
    ...
    public function datalist($where, $param)
    {
        ...
        try {
			...
			->each(function ($item, $key){
				...
				$item['start_time_str'] = to_date($item['start_time'],'Y-m-d');
				$item['end_time_str'] = to_date($item['end_time'],'Y-m-d');
    ...
    */
    public function getById($id)
    {
		...
		if($info['start_time'] > 0){
			$info['start_time_str'] = to_date($info['start_time'],'Y-m-d');
		}
		else{
			$info['start_time_str'] = '';
		}
		if($info['end_time'] > 0){
			$info['end_time_str'] = to_date($info['end_time'],'Y-m-d');
		}
		else{			
			$info['end_time_str'] = '';
		}
